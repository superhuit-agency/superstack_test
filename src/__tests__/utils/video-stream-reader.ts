 ...
 * @ignore
 */
export default class videoStreamReader extends EventEmitter {
	private page: Page;
	private options: PuppeteerScreenRecorderOptions;
	private sessionsStack: Array<CDPSession> = [];
	private isStreamingEnded = false;

	private isFrameAckReceived: Promise<void> | null = null;
	...
	private getCurrentSession(): CDPSession | null {
		if (this.sessionsStack.length > 0) {
	...
	private addListenerOnTabOpens(page: Page): void {
		page.on('popup', (newPage) => {
			if (newPage) {
				this.registerTabListener(newPage);
			}
		});
	}

	private removeListenerOnTabClose(page: Page): void {
		page.off('popup', (newPage) => {
			if (newPage) {
				this.registerTabListener(newPage);
			}
		});
	...
	private async handleScreenCastFrame(session: CDPSession) {
